...
export default function Home() {
  ...
  const [marque, setMarque] = React.useState("Toutes");
  const [carburant, setCarburant] = React.useState("Tous");
  const [boite, setBoite] = React.useState("Toutes");
  ...
  // Options filtres
  const marques = React.useMemo(() => ["Toutes", ...unique(VEHICULES.map(v => v.marque)).sort()], []);
  const carburants = React.useMemo(() => ["Tous", ...unique(VEHICULES.map(v => v.carburant)).sort()], []);
  const boites = React.useMemo(() => ["Toutes", ...unique(VEHICULES.map(v => v.boite)).sort()], []);
